...
from RobotCommandParser.CoreferenceUtils import coref_ops, util
#import conll
#import metrics
#import optimization
from RobotCommandParser.CoreferenceUtils import bert_tokenization as  tokenization
from RobotCommandParser.CoreferenceUtils import bert_modeling as modeling
from RobotCommandParser.CoreferenceUtils.pytorch_to_tf import load_from_pytorch_checkpoint
import logging
...
class CorefModel(object):
      ...
      while True:
        ...
        if self.config['single_example']:
          for example in train_examples:
            num_words = sum(len(s) for s in example["sentences"])
            if num_words>2000:
              continue
            try:
                tensorized_example = self.tensorize_example(example, is_training=True)
            except Exception as e:
              logging.error("Can't tensorize example in _enqueue_loop:\n{}\n{}".format(str(e),str([example])))
              raise e
              tensorized_example = None
  ...
  def get_predictions_and_loss(self, input_ids, input_mask, text_len, speaker_ids, genre, is_training, gold_starts, gold_ends, cluster_ids, sentence_map):
    ## это уже было закоменчено в 2020 with tf.device('/device:gpu:0'):
  ...
  def load_eval_data(self):
    if self.eval_data is None:
      def load_line(line):
        ...
        try:
          tensorizedSample = self.tensorize_example(example, is_training=False)
        except Exception as e:
          loggin.error("Can't tensorize example in load_eval_data\n"+str(example))
          raise e
          return None, example

        return tensorizedSample, example
  ...
  def evaluate(self, session, global_step=None, official_stdout=False, keys=None, eval_mode=False):
    ...
    for example_num, (tensorized_example, example) in enumerate(self.eval_data):
      try:
        ...
        logging.error("can't tensorize example in evaluate" + str(example))
        predicted_antecedents = [-1]# * len()
        top_span_starts = []
        #logging.debug("example num: {}\n{}".format(str(example_num),str(tensorized_example)))
        #raise e
